...
(function () {
  ...
  function toNetto() {
    ...
    const nettoByBl = {};
    _blFlows.forEach(bl => { nettoByBl[bl.name] = bl.zuzug - bl.wegzug; });
    ...
    setTimeout(() => {
      ...
      setTimeout(() => {
        ...
        _dotsG.selectAll('text.netto-label')
          ...
          .attr('y', function (d) { return (yScaleNew(d.name) ?? 0) + bw / 2; });
  ...
  function setupScroller() {
    ...
    scroller
      ...
      .onStepExit(({ element, direction }) => {
        ...
        if (step === 'bezirke') { _curStep = 'map';     toMap();     }
        if (step === 'chart')   { _curStep = 'bezirke'; toBezirke(); }
        if (step === 'flows')   { _curStep = 'chart';   toChart();   }
        if (step === 'netto')   { _curStep = 'flows';   toFlows();   }
      });
